...
Here it follows a simple and qualitative comparison between the analytical and numerical solution on a single uniform mesh, using $N_{el} = 2^{11}$, which shows that the numerical solution approximates well the analytical solution.
...
This trend \textit{predicts} the rapid appearance of numerical errors as the mesh size gets smaller, highlighting that one may not refine the whole mesh when not needed.
